...
def render_page(current_index, images):
    ...
    if current_index < total_images - 1:
        prev_button = f'<a href="/view/{current_index + 1}" class="nav-button">← Image précédente</a>'
    else:
        prev_button = '<span class="nav-button disabled">← Image précédente</span>'
    
    next_button = ""
    if current_index > 0:
        next_button = f'<a href="/view/{current_index - 1}" class="nav-button">Image suivante →</a>'
    else:
    ...
    <body>
        <div class="container">
            ...
            <div class="image-container">
                <img src="/image/{current_index}" alt="{current_image['name']}" class="main-image" id="mainImage">
            </div>
            
            <div class="navigation">
                {prev_button}
                <a href="/" class="nav-button">🏠 Dernière image</a>
                {next_button}
            </div>
        ...
        <script>
            ...
            document.addEventListener('keydown', function(e) {{
                switch(e.key) {{
                    case 'ArrowLeft':
                        {f"window.location.href = '/view/{current_index - 1}';" if current_index > 0 else ""}
                        break;
                    case 'ArrowRight':
                        {f"window.location.href = '/view/{current_index + 1}';" if current_index < total_images - 1 else ""}
                        break;
                    case 'Home':
